...
@objc public protocol AKPickerViewDelegate: UIScrollViewDelegate {
    optional func pickerView(pickerView: AKPickerView, didSelectItem item: Int, animated: Bool)
...
public class AKPickerView: UIView, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, AKCollectionViewLayoutDelegate {
    ...
    /// Readwrite. Infinity scroll
    public var infinity = false
    
    ///Private items for infinity effect
    private let additionalItemsForInfinity = 2
    ...
    */
    ...
    private func selectItem(item: Int, animated: Bool, notifySelection: Bool) {
        var indexToScroll = item
        var animated = animated
        
        if infinity {
            if item == 0 {
                animated = false
                indexToScroll = countItemsInCollectionView() - additionalItemsForInfinity
            } else if item == (countItemsInCollectionView() - 1) {
                indexToScroll = 1
                animated = false
            }
        }
        ...
        self.collectionView.selectItemAtIndexPath(
            NSIndexPath(forItem: indexToScroll, inSection: 0),
            animated: animated,
            scrollPosition: .None)
        self.scrollToItem(indexToScroll, animated: animated)
        self.selectedItem = indexToScroll
        if notifySelection {
            self.delegate?.pickerView?(self, didSelectItem: infinityIndexFromRealIndex(item), animated: animated)
        }
    }
    
    private func countItemsInCollectionView() -> Int {
        return infinity ? dataSource!.numberOfItemsInPickerView(self) + additionalItemsForInfinity : dataSource!.numberOfItemsInPickerView(self)
    }
    
    
    //MARK: infinity
    
    private func infinityIndexFromRealIndex(index: Int) -> Int {
        guard infinity else {
            return index
        }
        var result = index - 1
        if index == 0 {
            result = countItemsInCollectionView() - additionalItemsForInfinity - 1
        } else if index == countItemsInCollectionView() - 1 {
            result = 0
        }
    ...
    */
    ...
    public func collectionView(collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return self.dataSource != nil ? countItemsInCollectionView() : 0
    }
    ...
    public func collectionView(collectionView: UICollectionView, cellForItemAtIndexPath indexPath: NSIndexPath) -> UICollectionViewCell {
        ...
        let index = infinityIndexFromRealIndex(indexPath.item)
        if let title = self.dataSource?.pickerView?(self, titleForItem: index) {
            ...
            if let delegate = self.delegate {
                delegate.pickerView?(self, configureLabel: cell.label, forItem: index)
                if let margin = delegate.pickerView?(self, marginForItem: index) {
                    cell.label.frame = CGRectInset(cell.label.frame, -margin.width, -margin.height)
                }
            }
        } else if let image = self.dataSource?.pickerView?(self, imageForItem: index) {
    ...
    public func collectionView(collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAtIndexPath indexPath: NSIndexPath) -> CGSize {
        ...
        if let title = self.dataSource?.pickerView?(self, titleForItem: index) {
            ...
            if let margin = self.delegate?.pickerView?(self, marginForItem: index) {
